      ...
      <section>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-6">
          {categories.map((cat, i) => (
            <Link key={i} href="/products" className="group flex flex-col items-center gap-3">
      ...
      <section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {bestDeals.map((product) => (
            <div key={product.id} className="bg-white rounded-xl border border-gray-200 p-4 hover:shadow-lg transition-all group relative flex flex-col">
              ...
              <Link href={`/products/${product.id}`} className="aspect-square w-full mb-4 flex items-center justify-center bg-gray-50 rounded-lg overflow-hidden">
                <img src={product.image} alt={product.name} className="object-cover h-full w-full group-hover:scale-110 transition-transform duration-300 mix-blend-multiply" />
              </Link>
              <div className="space-y-2 flex-1">
                <Link href={`/products/${product.id}`}>
                  <h3 className="font-semibold text-gray-900 truncate hover:text-amber-500">{product.name}</h3>
                </Link>
              ...
              <button className="w-full mt-4 bg-gray-100 hover:bg-amber-400 text-gray-900 font-medium py-2 rounded-lg transition-colors text-sm">
                Add to Cart
              </button>
            </div>
          ))}
